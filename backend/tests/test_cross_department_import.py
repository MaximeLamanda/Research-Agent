...
async def test_city_anchor_overrides_hallucinated_run_department(db_session):
    ...
    fake_search = [{"url": "https://example.com/lyon", "title": "Amazon Lyon", "score": 0.9}]
    fake_fetch = [{"url": "https://example.com/lyon", "title": "Amazon Lyon", "text": "x" * 120}]
    emit_mock = AsyncMock()
    project_mock = SimpleNamespace(id=uuid4(), name="Amazon logistics warehouse")
    upsert_mock = MagicMock(return_value=(project_mock, True))
    ...
    processed = db_session.query(ProcessedUrl).filter(ProcessedUrl.url == "https://example.com/lyon").first()
    assert processed is not None
    assert processed.reason == "cross_department"
